...
class YOLO_detector(object):
    ...
    def postprocess(self, net_out):
        meta = self.meta
        result = lib.box_constructor(meta, net_out)
        return result
...
def demo_image():
    ...
    img_dir = "./test"
    image_names = lib.find_files(img_dir)
    for filename in image_names:
        ...
        for i in range(len(results)):
            cv2.putText(draw, str(results[i]['category']), (int(w * results[i]['x1']), int(h * results[i]['y1']) - 12),
                        0, 1e-3 * h, colors[results[i]['label']], thick // 3)
            cv2.rectangle(draw, (int(w * results[i]['x1']), int(h * results[i]['y1'])),
                          (int(w * results[i]['x2']), int(h * results[i]['y2'])), colors[results[i]['label']], thick)
...
def demo_video():
    ...
    while True:
        ...
        for i in range(len(results)):
            ...
            cv2.putText(draw, "{},{}".format(str(results[i]['category']), results[i]['score']),
                        (int(w * results[i]['x1']), int(h * results[i]['y1']) - 12), 0, 1e-3 * h,
                        colors[results[i]['label']], thick // 3)
            cv2.rectangle(draw, (int(w * results[i]['x1']), int(h * results[i]['y1'])),
                          (int(w * results[i]['x2']), int(h * results[i]['y2'])), colors[results[i]['label']], thick)
        cv2.imshow("result", draw)
        cv2.waitKey(25)
